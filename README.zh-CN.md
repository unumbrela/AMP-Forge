...
AMP Forge 是一个抗菌肽（AMP）生成项目，目标是构建可落地、可扩展的完整流程，覆盖：
...
- 项目页面：[https://unumbrela.github.io/amp-research2/](https://unumbrela.github.io/amp-research2/)
...
参考输出：`esm_diffvae/results/evaluation/evaluation_results.json`
...
仓库默认不跟踪大体积产物（checkpoint、processed data、embeddings、results 等）。

- 建议通过 GitHub Releases 或外部对象存储共享权重。
- 推送前可用脚本检查大文件：
